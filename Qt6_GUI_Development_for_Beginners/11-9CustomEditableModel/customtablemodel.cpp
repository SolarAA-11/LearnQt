...
{
    if(!parent.isValid())
        return table.empty() ? 0 : table.at(0).size();
    else
        return 0;
}

QVariant CustomTableModel::data(
...
QVariant CustomTableModel::headerData(
    int section,
...
{
    if(role != Qt::EditRole || table.empty() || value.toString().isEmpty())
        return false;
